...
def get_size_of_fused_ring(smi, includeSpiro=False):
    try:
        mol = Chem.MolFromSmiles(smi)
        ri = mol.GetRingInfo()
        systems = []
        for ring in ri.AtomRings():
            ...
            nSystems.append(ringAts)
            systems = nSystems
        number_of_fused_rings_list = ""
        for system in systems:
            frag_smi = Chem.MolFragmentToSmiles(mol, system, canonical=True)
            mol_frag = Chem.MolFromSmiles(frag_smi)
            number_of_fused_rings = len(mol_frag.GetRingInfo().AtomRings())
            # print("number of rings in frag",number_of_fused_rings)
            number_of_fused_rings_list += str(number_of_fused_rings)
        return number_of_fused_rings_list
    except:
        return None
