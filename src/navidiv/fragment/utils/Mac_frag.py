    ...
    for compats in bondMatchers:
        for i1, i2, bType, patt in compats:
            ...
            for match in matches:
                if (
                    match not in bondsDone
                    and (match[1], match[0]) not in bondsDone
                ):
                    bond = mol.GetBondBetweenAtoms(match[0], match[1])

                    if not bond.IsInRing() or (
                        bond.IsInRing() and SSSRsize_filter(bond, maxSR=maxSR)
                    ):
                        bondsDone.add(match)
...
def Get_block_index(blocks):
    ...
    for bs in blocks:
        tmp = [
            a.GetAtomMapNum() for a in bs.GetAtoms() if a.GetSymbol() != "*"
        ]
        #        tmp=list(set(tmp))
...
def MacFrag(mol, maxBlocks=6, maxSR=8, asMols=False, minFragAtoms=1):
    ...
    maxBlocks = min(maxBlocks, len(blocks))
    if maxBlocks == 1:
        if asMols:
    ...
    for b in bonds:
        ba1, ba2 = b[0]
        edges.append(
            (block_index[bond_block[ba1]], block_index[bond_block[ba2]])
        )
...
def write_file(input_file, dir, maxBlocks, maxSR, asMols, minFragAtoms):
    if ".smi" in input_file:
        smiles = [
            line.strip().split()[0] for line in open(input_file).readlines()
        ]
